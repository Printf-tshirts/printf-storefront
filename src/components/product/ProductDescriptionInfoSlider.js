...
const ProductDescriptionInfoSlider = ({
  ...
  const [selectedProductColor, setSelectedProductColor] = useState(
    product.variation ? product.variation[0].color._id : "",
  );
          ...
          <div className="pro-details-color-wrap">
            ...
            <div className="pro-details-color-content">
              {product.variation.map((single, key) => {
                return (
                  <label
                    className={`pro-details-color-content--single ${single.color.name}`}
                    key={key}>
                    <input
                      type="radio"
        ...
        <div className="pro-details-meta justify-content-center">
          ...
          <ul>
            {product.category.map((single, key) => {
              return (
                <li key={key}>
                  <Link to={process.env.PUBLIC_URL + "/tshirts"}>{single}</Link>
                </li>
              );
            })}
          </ul>
